...
export const up = transaction(async (client: Client) => {
  ...
  await insert(mainCategories, client);
  await insert(hundredCategories, client);
  await insert(thousandCategories, client);
});
...
function deserialize(params: {
  ...
  for (const [decimal, name] of params.classes) {
    ...
    category.parentID = params.ancestor(decimal);
    categories.set(decimal, category);
  }
...
export const down = transaction(async (client: Client) => {
  await client.query("DELETE FROM dewey_category;");
});

const ddc_23_main_classes: [
  decimal: string,
  name: string
][] = [
...
const ddc_23_hundred_classes: [
  ...
  ["330", "Economics"],
  ["340", "Law"],
  ["350", "Public administration and military science Social"],
  ["360", "problems and social services"],
  ["370", "Education"],
...
const ddc_23_thousand_classes: [
  ...
  ["257", "[Unassigned]"],
  ["258", "[Unassigned]"],
  ["259", "Pastoral care of families & groups of people"],
  ...
  ["628", "Environmental & sanitary engineering"],
  ...
  ["683", "Hardware, weapons & household appliances"],
